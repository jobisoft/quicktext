...
try {
  var { cardbookUtils } = ChromeUtils.import("chrome://cardbook/content/cardbookUtils.js");
} catch(e) {}
...
,
  getcarddata_from: function(aData, aIdentity)
  {
    ...
    try {
      let card = cardbookUtils.getCardFromEmail(aIdentity.email.toLowerCase());
      if (card)
      {
        ...
        aData['FROM'].data['fullname'] = TrimString(cardbookUtils.getName(card));
        aData['FROM'].data['title'] = TrimString(card.title);
        aData['FROM'].data['workphone'] = TrimString(cardbookUtils.getCardValueByField(card, "tel.0.worktype", false));
        aData['FROM'].data['faxnumber'] = TrimString(cardbookUtils.getCardValueByField(card, "tel.0.faxtype", false));
        aData['FROM'].data['cellularnumber'] = TrimString(cardbookUtils.getCardValueByField(card, "tel.0.celltype", false));
        aData['FROM'].data['custom1'] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM1", false));
        aData['FROM'].data['custom2'] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM2", false));
        aData['FROM'].data['custom3'] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM3", false));
        aData['FROM'].data['custom4'] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM4", false));
...
,
  getcarddata_to: function(aData, aIndex)
  {
    let passStandardCheck = false;
    try {
      let card = cardbookUtils.getCardFromEmail(aData['TO'].data['email'][aIndex]);
      if (card)
      {
        aData['TO'].data['firstname'][aIndex] = TrimString(card.firstname);
        aData['TO'].data['lastname'][aIndex] = TrimString(card.lastname);
        aData['TO'].data['fullname'][aIndex] = TrimString(cardbookUtils.getName(card));
        ...
        aData['TO'].data['workphone'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "tel.0.worktype", false));
        aData['TO'].data['faxnumber'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "tel.0.faxtype", false));
        aData['TO'].data['cellularnumber'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "tel.0.celltype", false));
        aData['TO'].data['custom1'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM1", false));
        aData['TO'].data['custom2'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM2", false));
        aData['TO'].data['custom3'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM3", false));
        aData['TO'].data['custom4'][aIndex] = TrimString(cardbookUtils.getCardValueByField(card, "X-CUSTOM4", false));
        passStandardCheck = true;
        }
    } catch(e) {}
